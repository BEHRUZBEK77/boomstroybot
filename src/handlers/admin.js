...
const { fmtNum, fmtDate, orderStatusLabel, getStockStatus, totalStock, addStock } = require('../utils/helpers');
    ...
    for (const item of order.items) {
      ...
      if (prod) {
        await updateDoc('products', item.productId, {
          stock: addStock(prod.stock, item.qty),
        });
      }
    }
...
async function handleAdminLowStock(ctx) {
  ...
  const text =
    ...
    low.map(p => {
      ...
      return `${st.emoji} ${escapeMarkdown(p.name)}\n   Qoldi: ${totalStock(p)} ${p.unit || 'dona'} (min: ${p.minQuantity || 5})`;
    }).join('\n\n');
