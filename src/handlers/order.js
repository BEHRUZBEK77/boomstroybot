...
const { fmtNum, fmtDate, generateOrderNumber, orderStatusLabel, deductStock } = require('../utils/helpers');
const {
...
// ─── STEP 1: Buyurtmani boshlash → yetkazib berish yoki borib olishni tanlash ─
async function handleOrderStart(ctx) {
  ...
  setSession(ctx.from.id, { orderTotal: total, deliveryType: null });

  const { receiveTypeChoice } = require('../utils/keyboards');
...
async function handleReceiveDelivery(ctx) {
  ...
  try { return await ctx.editMessageText(t(lang, 'chooseAddress'), { parse_mode: 'Markdown', ...kb }); }
  catch { return await ctx.reply(t(lang, 'chooseAddress'), { parse_mode: 'Markdown', ...kb }); }
}

// ─── STEP 1b: "Borib olish" tanlandi → omborlar ro'yxatini ko'rsatish ────────
async function handleReceivePickup(ctx) {
  const lang = langOf(ctx);
  await ctx.answerCbQuery();
  ...
  if (!all.length) {
    await ctx.editMessageText(t(lang, 'pickupNoWarehouses'), { parse_mode: 'Markdown' });
    return handleOrderStart(ctx);
  }

  const { pickupWarehouseButtons } = require('../utils/keyboards');
  await ctx.editMessageText(t(lang, 'pickupChoose'), {
    parse_mode: 'Markdown',
    ...pickupWarehouseButtons(all, lang),
  });
}

// ─── STEP 1c: Borib olinadigan ombor tanlandi → to'lov turiga o'tish ─────────
async function handlePickupWarehouseSelect(ctx, warehouseId) {
  const lang = langOf(ctx);
  await ctx.answerCbQuery();

  let wh;
  if (warehouseId === 'main') {
    wh = { id: 'main', name: WAREHOUSE.name, address: WAREHOUSE.address || '', phone: WAREHOUSE.phone || '' };
  } else {
    const { getCollection } = require('../services/firebase');
    const list = await getCollection('warehouses').catch(() => []);
    wh = (list || []).find(w => w.id === warehouseId) || { id: warehouseId, name: warehouseId };
  }
  ...
  const text = t(lang, 'pickupSelected', {
    wh: wh.name,
    addr: wh.address ? `📌 ${wh.address}\n` : '',
    phone: wh.phone ? `📞 ${wh.phone}\n` : '',
    total: fmtNum(total),
    grand: fmtNum(total),
    som: t(lang, 'som'),
  });
    ...
    for (const item of cart) {
      ...
      if (prod) {
        await updateDoc('products', item.productId, {
          stock: deductStock(prod.stock, item.qty),
        }).catch(() => { });
      }
    }
  } catch { }
