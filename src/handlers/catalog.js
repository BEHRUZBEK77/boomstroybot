...
const { getStockStatus, fmtNum, totalStock } = require('../utils/helpers');
...
async function handleCategoryCallback(ctx, catId, page = 0) {
  ...
  const available = products.filter(p => p.status !== 'inactive' && totalStock(p) > 0);
...
async function handleProductCallback(ctx, productId) {
  ...
  const text =
    ...
    ratingLine +
    `📦 ${t(lang, 'available')}: ${totalStock(p)} ${unit}\n` +
    (p.description ? `\n📝 ${p.description}` : '') +
    `\n━━━━━━━━━━━━━━━\n` +
